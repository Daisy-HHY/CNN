"""
脚本1: 训练教师模型 (TeacherCNN)

CPU优化版, ~930K参数, ~2-3min/epoch
用法: python train_teacher.py --epochs 30
"""
import argparse
import csv
import os
...
from config import DEVICE, TEACHER, get_device_info
from models.resnet import TeacherCNN, count_parameters
...
def main():
    parser = argparse.ArgumentParser(description="Train Teacher TeacherCNN")
    ...
    print("=" * 60)
    print("  Training Teacher: TeacherCNN (CIFAR-10)")
    print("=" * 60)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if test_acc > best_acc:
            best_acc = test_acc
            torch.save(model.state_dict(), TEACHER["checkpoint"])
    ...
    print(f"  Saved: {TEACHER['checkpoint']}")
